...
public class OrangeDataReader : CapDataReader
{
	...
	public void LoadExtraData(string slicedRawData, string extendRawData, List<string> allowedExtendTable)
	{
		try
		{
			...
			try
			{
				...
				{
					Debug.Log("載入企劃切割資料");
					ReadServerData(slicedRawData, registCRC: false, null, forceReplaceTable: false);
					flag = true;
					flag2 = true;
				}
				...
				{
					Debug.Log("載入營運延伸資料");
					ReadServerData(extendRawData, registCRC: false, allowedExtendTable, forceReplaceTable: false);
					flag2 = true;
				}
			}
			catch (Exception ex)
	...
	public void LoadExtData(string slicedRawData)
	{
		try
		{
			...
			ReadStream(encryptedData, null, forceReplaceData: false);
		}
		catch (Exception ex)
		{
			...
			MonoBehaviourSingleton<OrangeWebRequestLoad>.Instance.Load(OrangeWebRequestLoad.LoadType.OPERATION_TABLE, p_fileName, OrangeWebRequestLoad.LoadingFlg.TEXT_DEFAULT, delegate(byte[] p_param0, string p_param1)
			{
				if (p_param0 != null)
				{
					...
					try
					{
						...
						ReadStream(p_param0, list, forceReplaceData: false);
					}
					catch (Exception ex)
					{
	...
	private void RegistCRC(string name, uint crc32)
	{
		...
		Debug.Log($"RegistCRC Target={name} CRC={crc32}");
	}
	...
	public IEnumerator LoadForbiddenInfo()
	{
		...
		while (!isForbiddenLoaded)
		{
			...
			{
				Debug.Log($"Download {forbiddenFile} failed!");
				break;
			}
